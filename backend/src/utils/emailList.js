// Recipient-list parsing for admin marketing campaigns.
//
// Two entry points, one output shape:
//   parseEmailList(text)  - a pasted list ("a@x.com, b@y.com")
//   parseEmailCsv(text)   - the text of an uploaded .csv
//
// Both are pure and dependency-free ON PURPOSE: frontend/src/utils/emailList.js
// is a copy of the same logic so the admin portal can preview a list without a
// round trip. This file is the authoritative one - the controller re-parses
// whatever the browser posts and never trusts the client's counts. If you edit
// one, edit the other; tests/emailList.unit.test.js guards the behaviour.
